...
== Arduino 化の目的
前著「レーザー本総集編」および本書の前半部においては、描画データの生成をPCで行っていた。動画や任意の画像 を用いて描画するといった事を考えた場合にはPCを利用することは非常に優位性があるが、消費電力が大きいという問題がある。現状描画しているアナログ時計程度のものであれば  計算力も小さいため、Arduino等のマイコンを用いることで大幅に消費電力低減とシステム の簡素化・小型化ができると考えた。そこで現状のパソコン制御をやめてArduinoを用いたデモシステムを制作することにした。

=== 従来システム
従来デモに使用していたノートパソコンはDell Mini9であり、CPUはAtom N270 Diamondville（ダイアモンドビル）1.6GHzの1コア2スレッドで、 CPU単体のTPDは2.5W である。ノートパソコン自体の消費電力は、システム全体としてはおよそ9W(フル充電時、純正AC アダプタ利用での実測値。サンワサプライワットチェッカーによる。) である。2014年ごろとしてもすでに割と型落ちではあるが、今となってはロートルにすぎる。

またデモ中の描画システム全体の消費電力15W(電源の電流モニタでの読み値。電源電圧を約12.5V、電流読み値約1.2Aから算出)であった。この消費電力の60%が制御用パソコンである状態は著しくバランスを欠くと言わざるを得ない。

そこで、制御部をArduinoとしたレーザー時計を作ることとする。設計の構想は以下のとおりである。

@<em>{RTCから時刻を取得し、レーザーで時計の描画を行う。}文字にすれば25 文字であるが、ここから順次実装について述べていく。

== Arduino 事始め

まず購入したのはArduino Unoであり、実施当時のIDE(開発環境)のバージョンは1.6.5である。秋月で2940円であった。Lチカまでは非常にスムーズかつ簡単であったことを申し添えておく。ただし、やはりこの手の開発において、L チカの次に何を作るかという問題は非常に重要であり、「何かを作りたいからデバイスを買う」という風にモチベーション付をする必要があり、デバイスを買ってから何を作るか考えるという状態では積んでしまうこと請け合いと考える。さて、Arduinoでレーザー時計を実装するためのステップは以下のとおりである。

 * 液晶の取り付け

I@<sup>{2}C @<fn>{I2C}のテストと、出力結果の確認のため、I@<sup>{2}C 接続の小型液晶を付ける。

 * RTC(Real Time Clock) の取り付け

描画する現在時刻の基準として使用する。これもI@<sup>{2}C 接続である。
...
//footnote[I2C][https://ja.wikipedia.org/wiki/I2C アイツーシーと読むことも多い。2線シリアル通信の規格の一つ。電子工作界隈でもよく使われる。]

このようにして、実装を進める。秋月でも安価に売っているI@<sup>{2}C 接続の小型液晶を使用して、Arduino にモニタを付ける。液晶画面には、現在時間を表示する。

使用した液晶はAQM0802A-RN-GBW2@<fn>{lcd}である。8文字× 2段の小型キャラクタ液晶で、 700円程度と非常に安価である。配線は、電源とGNDとI@<sup>{2}Cの2線の合計4線のみ配線すればよく、配線も簡単である。これを用いて、時刻などをもろもろ表示させる。
...
図10-1はブレッドボードとジャンパワイヤでとりあえずのテスト実装したところである。 Arduino側はピンソケットになっているので、ジャンパ線を差し込む。液晶側はブレッドボードに差し込み、その隣にジャンパ線を差し込む。
...
当初テストした時は全く何も表示されなかったので、ハンダ付けで壊したか？と思ったが、なんのことはない。配線を間違っていただけであった。I@<sup>{2}C接続は、操作、実装が非常に簡単になるメリットが有る一方で、電源を入れても、正しい信号を送らないと全く何の表示もなされないから、表示されない場合に故障なのか設定ミスなのか何なのかさっぱりわからないというデメリットある。電源入れたら何かしらパターンでも全点灯して消えるくらいのアクションがあればいいのに・・・と思う。いずれにせよ、配線が汚いとか、そのあたりはご容赦いただくとして、次のテストに進む。

@<list>{code1}に、ストップウォッチ的に、ただ1秒毎に数字をカウントアップするコードを書いた。次に実装するRTCから時刻を取ってきて表示する前準備として実施した。いずれにせよ、たったこれだけのコードで液晶が使えるのはありがたいことである。

//listnum[code1][Code10-1:I@<sup>{2}C接続の液晶の表示サンプル1]{
#include <ST7032.h> 
#include <Wire.h> 
ST7032 lcd;
void setup() {
...
//listnum[code2][Code10-2:カウントアップするコード]{
    void loop() {
...
これも基本的には他の開発例をそのまま実装するだけである。RTC8564の使い方@<fn>{rtc1} @<fn>{rtc2}を参照しつつ、ライブラリをダウンロード@<fn>{rtcdownload}する。書式が少し変わっているので、Arduino IDE が最近のものならば少し修正が必要である。解凍して、RTC8564.cpp に対して以下の修正を行う。テキストエディタで一括変換したほうが楽かつ漏れがない。
...
続いて、シリアルモニタではなく、液晶に表示させるスケッチを@<list>{code4}のように実装する。基本的な記述はたいした内容ではなく、RTCから取得した時刻を表示ごとにlcd.print() で液晶画面に表示するだけである。なお、@<list>{code4}ではそれぞれ、時、分、秒について、一桁の範囲では桁数がずれてしまうため桁数を合わせる操作も行っている。具体的には、取得した値が 10 未満であればあたまに0を付加するという操作である。
 ...
 * RTCの保持用　電気二重層キャパシタ 5.5V1.5F@<fn>{akicap}
 * 逆流防止用ダイオード（とりあえず手元にあった1S3 30V1A の整流用ダイオード）@<fn>{diode}
 * メイン基板Arduino 用ユニバーサル基板 ガラスコンポジット（赤）@<fn>{board}

ユニバーサル基板のPDF 適当に拾ってきて、印刷して、手書きでパーツの配置と配線ルートを考える。ものを適当に並べてみて、配線を適当に引く。大きいものから並べて、展示・設置の方向も考えて、端から置いていく。次に配線をする。両面スルーホールなので配線をまたぐのは極力減らしたい、という思想で配線をする。I@<sup>{2}Cデバイスに必要な配線は、VCC、 SDA、SCL、GND の4 線である。
...
1の不具合原因はSCLとVCCが交差するところで、裏と表のパターンが交差するのだが、スルーホールを介してショートしていたためである。カプトンを貼って応急処置した。このときArduino用基板といっしょになんとなく買ったカプトンテープ@<fn>{capton}がその日のうちに早速役に立ったこととなる。
...
=== DA 方法の検討
いよいよ本題のArduinoを使ったDAの検討にうつる。ガルバノに入力するアナログ信号を出力するため、DA(Digital to Analog)を行う。DA の方法は2 つあり、一つは本体内蔵のPWMを利用したアナログ出力、もう一つはDAC-IC を使ってアナログ出力する方法である。当然それぞれメリット・デメリットがあるので、整理する。
...
AD 出力のスケッチを@<list>{code5}に示す。といっても、なんら特殊なものではなく、どこにでも書いてある一般的なものである。ピンを指定し、analogWrite(pin,duty)で出力する。引数はピンNoとDuty比である。Duty比は0～255の256 階調で、およそ0.4%刻みとなる。出力電圧は電源電圧とほぼ同じ5Vである。Duty設定値とオシロで測定したDuty比とrms値(Root means square。ほぼ実効値と考えて良い) の関係を図10.5 に示す。Duty はほぼ直線比例だが、rmsは非線形的に上昇する。
...
C とR が大きいほどより平滑化されることがわかる。また、振幅はカットオフ周波数において@<m>{1/\sqrt(2)}倍すなわち-3dBとなる。また透過する振幅はカットオフ周波数以下で入力信号のほぼ1倍となり、カットオフ周波数以上では、-6dB/oct(あるいは同じ意味であるが)-20dB/decとなる。周波数2倍でゲインが1/2、1桁上がる(=10 倍)で1/10となる。ただし、今回の目的はガルバノスキャナ制御用信号の生成であるから、あまり平滑化をし過ぎるとDC成分しか含まれない事になり、制御に使えなくなる。

ArduinoのPWMのキャリア周波数は490Hzなので、これに合わせて平滑化のためのRCフィルタを組んでみる。使ったのは手元にあった1kΩと1uFである。適当に組み合わせて、フィルタを作ることを考えた。まずは、1kΩ、1uFを1個づつ使ったフィルタのゲイン特性を見てみる。

1kΩ、1uFを使った時のカットオフ周波数を計算すると159Hz となり、ArduinoのPWM出力を平滑化できそうな事がわかる。最近はネット上にも便利なツールがたくさんあり、特性値を入力するだけで伝達関数を計算してカットオフ周波数Bode線図のグラフまで出力してくれるサイト@<fn>{keisan}がある。下の図は当該サイトで計算し、出力のボード線図を拝借したものである。縦軸が大きいのでわかりづらいが、100Hzより少し上で落ち始め、その後-20dB/dec で落ちている事がわかる
...
図10.8に1kΩ、1uF を使ったカットオフ159Hzのフィルタを適用したDuty比50.2%の波形を入力してみたところを示す。

電源電圧4.78Vに対し、平均値で2.41Vとほぼ半分、残っているゆらぎの成分は最大値3.56V、最小値1.24Vと2.32Vの振幅を持つことがわかった。流石にこの振幅は大きすぎるので、まずはコンデンサ容量を増やしてみる。1kΩと3uFを使うと、カットオフ周波数は 53Hzになる。その波形を図2-9に示す。平均値は変化せず2.41V、変動振幅は0.88Vまで低減した。また、5kΩ,3uF(カットオフ周波数=10.6Hz)では、変動の振幅は0.2Vまで抑えられる。ただし、あまりカットオフを下げ過ぎると今度は通過させたい信号が低下する事になる。したがって、実信号で試す必要がある。
...
図10-10にRCフィルタの実装例を示す。アナログ出力を一旦ブレッドボードで受け、RCを適当に接続し、出力部分にプローブを接続する。特性確定するまでの試行であるから、暫定的なものである。
...
次に、RTC と液晶を載せたユニバーサル基板上にフィルタを実装する。回路図と実装状況を図10-12、図10-13 に示す。カットオフは一旦低周波側とし、5kΩ（金皮）、4.7uF（積層セラミックコンデンサ）を使った。

次にArduino で時計を出力するために、アナログ出力部分のコーディングを行う。

== Arduinoを用いたアナログ時計の実装
過去、C84において、ContecのDAボードとPCを使った時計を作った。それをArduinoに移植する。改めて、基本的な考え方と実装の詳細を示す。
...
図10.16にXY2chそれぞれの波形を示す。上段の大きなカーブのところが周りの文字盤のところと推測されるが、下段(ch1)にV字が途中で折れている事がわかる。また、図10.17 に示すリサージュ表示においても、0 時のところと 3 時のところに謎の V 字が存在することがわかる。

これは、@<list>{codeArduinoClock}で「Dutyの引数が256階調だから」と振幅の引数を±128で実装したため、Duty=256=0となってしまったためである。そこで、+127に変更したところ、今度は6時と9時に-1が出てしまい、点対称な図形ができてしまった。面白いのは、Duty に-1を入れると、255相当に循環するというところか。
 
これは、単純なバグではあるが、見た目上の影響は重要なバグである。そこで与える振幅を127とし、1～255の範囲とした。

int(sin(2*M_PI*30*i/360)*127+128)
...
原点を@<list>{codeClockOrigin}のように与えているためである。
...
気づく過程として、以下の試行錯誤を経た。
 ...
 1. まず、取得している RTC をそのまま液晶に表示させてみた。lcd.print(Rtc.seconds(), HEX);当然そのまま表示される。

 2. なぜ HEX?と思いつく。 lcd.print(Rtc.seconds(), DEC）; と変更し 10 進数表示にしてみる。0～9 は良いが、10 を表示すべきところで 16 にジャンプすることがわかった。この挙動は、オシロで表示した時にジャンプする挙動に一致する。
