...
一方で実装の時間がほとんどとれなかったこともあり、準備の段階では解決したはずのRTC関係のバグが会場で再発したりしてしまった。しかも挙動がそれぞれ異なり、デブサミでは完全にバグっていて、0F:85:05といった感じの変な時刻を表示したり、技術書典2では、2:00:00で固まったりと、いずれもしょんぼりな動作であった。そこで、今回はそのバグの修正と、また描画にあたって、On-Off制御ができるようにしたい。

またガルバノの入力定格は±10VだがPWMで出力可能な電圧範囲は0-5Vであり、入力可能 電圧および描画可能範囲の1/4しか活用できていないことになる。そこで、本章では、レベルシフトやアンプなどの実装を行う。
...
なお回路図作図は、ド定番の回路図作成ソフト、Fritzingを今回使ってみた。前回はなんと手書き！結局基板CADの図か写真なのでこの後Fritzingの図は出てこないのだけど・・・

ソフトを使う場合、ソフトの使い方を覚えるとか、慣れるまでには結局かえって時間がかかるとかのデメリットがある。一方で、回路図を手書きしなくてよいのでむしろ早いのだが、複写が便利といったメリットがある。今回は、今後の活用も考え、こういう素敵ツールを使い始めることにした。

インストールは特に問題なく終了するのでFritzingを起動してみる。部品を配置して、結線して、うん、それっぽい。という具合で比較的簡単に使えることがわかった。また、部品配置図もカンタンに描ける。

本章では設計をしてみた、というだけなので、ON/OFF実装はのちの自分に期待！
...
=={chap12-DC} オペアンプ基板の試作とオペアンプ電源選定
描画の信号源はArduinoのPWM出力を平滑化して使用している。追加のデバイスが不要というメリットはあるが、制御周期と精度の問題がある。

また、ガルバノの描画可能領域をフルに使うには、入力電圧でマイナス側を使う必要がある。またガルバノの入力可能レンジは±10Vである。ところが、Arduinoの出力は0～+5Vであるから、半分程度しか利用できていないことになる。そこで、オペアンプを用いたレベルシフト回路を組むとともに±10Vまで生成可能な電源とオペアンプ増幅回路により。描画サイズを拡大する。単なるオペアンプ基板なら、他の用途にも使えるという計算もあってまずは単体のボードとして制作してみた。将来は、アンプ含めて1枚の基板として製作したい。

なお、これまでの実装@<hd>{chap10|chapPWMFreq}でPWMの高周波化@<fn>{pwmfreq}もしている。ArduinoのPWMの基本周波数は、実装例に多い標準の出力(IO-9、10ピン)では490Hzに対して、IOの5、6ピンは977Hz が標準の周波数である。そこで、現在実装中のArduino時計は、5番6番ピンを使って、制御周期 100Hzで動作させている。十分な高周波であれば平滑化しても問題ないのだが、やはり波形のなまりも気になる。と思っていたが、実際描画してみるとあまりそれほどでもない、という微妙なところであった。とはいえ、汎用性と、楽しい実装のことを考えると、ここらでDAチップを使ってきちんと出力をすることとする。
...
次に、実装済とするかどうかであるが、表面実装部品はないので、はんだ付けは購入者の方にやって頂く形としたい。面倒だけど、自分で作る部分がなければこんなの買ってもしょうがないしね。ということで、実装・製作を進める。
