...
次に、RTC(Real Time Clock)を実装する。使用するRTCは、過去の実装と同じく、秋月の RTC-8564NBを用いる。こちらもI@<sup>{2}C接続のRTCで、8ピンDIP変換済モジュール@<fn>{52}が市販されている。
...
== NTPを使う
Network Time Protocol(ネットワークタイムプロトコル:NTP)は、ネットワークに接続される機器において、機器が持つ時計をNTPサーバーの時刻に同期させるための通信プロトコルである。

用sるに、ネットワーク経由で時刻合わせをする機能である。最近のOSには標準機能として入っているが、昔から様々なNTPクライアントが存在したのでWindowsにおいてOS標準で取り込まれたのは最近なのかもしれない。

まずは、ESP32のWiFiを使ってNTPにより時刻同期し、その時刻をもとにアナログ出力を行う。今までRTCは乗せていたが、リセット時(あるいは起動時)に決め打ちした時刻を設定するものであり、リセットすべき時刻をハードコーディングしておき、その指定時刻(例えばコミケデモでは開場の10時)にリセットする必要があった。それを解決するのがNTPである。

通常のArduinoならネットワークへの接続も一苦労であったが、ESP32なら標準でWiFiが付いているのですぐ使用することができる。そこでまずはNTPによる時刻設定を実装する。

NTPを使うには、WiFiへの接続とNPTサーバーへのアクセスが必要になる。

WiFiに接続し、繋がったらNTPの同期をトライする。さて、ここでハタと気づいてしまった。NTPあればRTCいらなくない？内部のインクリメントだけでよい？という話である。

もちろん基本的には、RTCの方が精度が高いし、インクリメントでは様々なオーバーヘッドが保証されないので、NTPで初期化してRTCを使うという方針でいくことにした。

====[column][WiFiの設定にずいぶんハマった話]
当初、自宅環境のWiFiに接続していたのだが、WiFiには接続できるのだがNTPで同期ができないという問題に悩まされた。

結果として、接続先を変更したら一発で成功した。

ESP32のようなマイコンでは表示する機能/内容も少ないので問題切り分けに時間がかかることが多い。PCであればエラーメッセージを出すのは簡単だが、マイコンでは制約がある。それでも、シリアルモニタをつないでおくなどが対策となろう。

今回に関しては、NTPサーバーからのレスポンスがないというエラーによるもので、原因はポートが閉じられていたためであった。

接続できないというエラーが表示されるなら解決は簡単だし、時刻が表示されないならそれはそれで切り分けが簡単なのだが、今回は、タイムアウトしてRTCの初期値(1970年1月1日　9:00:00)が入るというエラーだったため、切り分けに苦労した。

//listnum[NTPerror][NTPエラーログ]{
#. CONNECTED
Conneted, IP adress: 192.168.3.10
Transmit NTP_Request
sendNTPpacket : ntp.nict.jp
No NTP Response :-(
ConfigNTPJST : 1970-01-01 09:00:06
DisCONNECTED
JST : 1970-01-01 09:00:17
//}

参考にしたHPに作例/コード例が載っていたとしても、「ポートを確認せよ」といった書き方がされていないことも少なくない。参照元の落ち度であるというつもりはない。

実際問題、こういう細かいトラブルは直面してみないと全く分からないし、コードの間違いではないので解決には非常に手間がかかる。一見動いているが想定した動きではないとき、かつ実装部分と異なる部分に原因があるため、本体で試行錯誤しても解決しない。悲しいことにたいていこういうトライをしていると、部品を壊したり、動いていたところまで動かなくなったりなんてことが往々にして発生する。

こういうこともありえる、ということを記載しておくことで、注意喚起や参考になれば幸いである。
