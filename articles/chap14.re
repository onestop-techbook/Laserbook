...
==	液晶と RTC の再実装
次は、I@<sup>{2}Cの実装を行う。この本を読んでいる方には言うまでもないことかもしれないが、 I@<sup>{2}Cとはシリアル通信規格の一つで、2本の信号線で通信が可能であり、実装が簡単という特徴がある。Arduino等の組み込み、マイコンの分野において、UART、SPIとともに非常に広く利用されている規格である。

ESP32のI2Cのデフォルトピンは、SDA＝IO21、SCL=IO22である@<fn>{48}。ArduinoのI2Cの関数Wire.begin()は引数を持っていて、任意のピンを使ったI@<sup>{2}C通信が可能とのことだが、特に変更する必要もないので標準のまま使用する。
...
RTCのライブラリ@<fn>{53}も公開されていて、こちらも簡単に使用する事ができる。 RTCから時刻を引っ張ってきて、液晶に表示する部分は以下のとおり。
