...
なお、これを実装している途中にパワーフラグの扱いがよくわからないという事態に直面した。例えば、この図で言うところの、オペアンプの負電源側のパスコンのGNDは、パワーフラグを立てておかないと、ERCの未接続エラーが出る。一方で、オペアンプの正電源のほうは、パスコン、GNDの接続のみで特にERCエラーはでない、この辺は、原因がわかったらまとめるかもしれない。現状は、エラーが出たらパワーフラグを立てておき、ERCエラーを消す、という場当たり的対応である。
